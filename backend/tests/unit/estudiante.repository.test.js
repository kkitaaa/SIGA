...
describe("EstudianteRepository", () => {
  ...
  test("busca estudiante por rut", async () => {
    ...
    await expect(EstudianteRepository.findByRut("12345678-9")).resolves.toEqual(
      { id_estudiante: 1 },
    );
  });
  ...
  test("actualiza estado NEE y datos de estudiante", async () => {
    ...
    await expect(EstudianteRepository.findById(1, prismaMock)).resolves.toEqual(
      { id_estudiante: 1 },
    );
    await expect(
      EstudianteRepository.updateNeeStatus(1, true, prismaMock),
    ).resolves.toEqual({ id_estudiante: 1 });
    await expect(
      EstudianteRepository.update(1, { es_nee: false }, prismaMock),
    ).resolves.toEqual({ id_estudiante: 1 });
  });
});
