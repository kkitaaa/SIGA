...
describe("repository branches", () => {
  ...
  test("AsignacionPieRepository cubre todos los métodos con y sin transacción", async () => {
    ...
    await expect(repo.findActiveByStudentExcluding(2, 5)).resolves.toEqual({ id: 1 });
    await expect(repo.create({ idEstudiante: 2, idFuncionario: 5 })).resolves.toEqual({ id: 1 });
    ...
    await expect(repo.findActiveByStudentExcluding(2, 5, tx)).resolves.toEqual({ id: 1 });
    await expect(repo.create({ idEstudiante: 2, idFuncionario: 5 }, tx)).resolves.toEqual({ id: 1 });
  ...
  test("EstudianteRepository cubre métodos estáticos con y sin transacción", async () => {
    ...
    await expect(EstudianteRepository.findByRut("123")).resolves.toEqual({ rut: "123" });
    await expect(EstudianteRepository.create({ rut: "123", primer_nombre: "A", primer_apellido: "B", sexo: "M", fecha_nacimiento: "2020-01-01", fecha_ingreso: "2020-01-01", id_curso: 1 })).resolves.toEqual({ id: 1 });
    await expect(EstudianteRepository.findAll()).resolves.toEqual([{ id: 1 }]);
    await expect(EstudianteRepository.findAllNee()).resolves.toEqual([{ id: 1 }]);
    await expect(EstudianteRepository.findById(1)).resolves.toEqual({ id: 1 });
    await expect(EstudianteRepository.updateNeeStatus(1, true)).resolves.toEqual({ id: 1 });
    await expect(EstudianteRepository.update(1, { es_nee: true })).resolves.toEqual({ id: 1 });

    await expect(EstudianteRepository.findById(1, tx)).resolves.toEqual({ id: 1 });
    await expect(EstudianteRepository.updateNeeStatus(1, false, tx)).resolves.toEqual({ id: 1 });
    await expect(EstudianteRepository.update(1, { es_nee: false }, tx)).resolves.toEqual({ id: 1 });
  });
  ...
  test("FuncionarioRepository cubre métodos con y sin transacción", async () => {
    ...
    await expect(FuncionarioRepository.findAllFuncionarios()).resolves.toEqual([{ id: 1 }]);
  });
});
