export class RegisterDTO {
  ...
  validate() {
    if (!this.nombre) throw new Error("VALIDATION_ERROR: El nombre es obligatorio");
    if (!this.email) throw new Error("VALIDATION_ERROR: El email es obligatorio");
    
    // Validacion formato de email
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(this.email)) throw new Error("VALIDATION_ERROR: El formato del email no es válido");
