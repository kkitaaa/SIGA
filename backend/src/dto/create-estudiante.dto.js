export class CreateEstudianteDTO {
  constructor(data) {
    ...
    this.fecha_ingreso = data.fecha_ingreso ? new Date(data.fecha_ingreso) : new Date();
  ...
  validate() {
    if (!this.primer_nombre) throw new Error("VALIDATION_ERROR: El nombre es obligatorio");
    if (!this.rut) throw new Error("VALIDATION_ERROR: El RUT es obligatorio");
    if (!this.id_curso) throw new Error("VALIDATION_ERROR: El curso es obligatorio");
  }
}